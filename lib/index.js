...
/** 替换式主路由（无感：模型选择器不用动）；其他文本路由走变体式（见下）。 */
export const SCOPE_PROVIDER = 'deepseek-official'

// =============================================================
// 自定义视觉后端（v1.2，A：用户可换视觉模型）
// =============================================================
/** 自定义后端凭证键（.credentials.yaml 可选字段；不填则用内置 BACKENDS）。
 *  优先级：custom 在尝试链最前，内置四家兜底。 */
export const CUSTOM_KEY = {
  baseURL: 'VB_BASE_URL',     // OpenAI 兼容端点（或 Gemini 端点）
  model: 'VB_MODEL',          // 模型名
  apiKey: 'VB_API_KEY',       // API key
  protocol: 'VB_PROTOCOL',    // openai（默认）| gemini
  needsProxy: 'VB_NEEDS_PROXY', // true 时走 VISION_PROXY（默认 false）
}
export const CUSTOM_BACKEND_NAME = 'custom'

/** 从凭证键表构建自定义后端配置；未配置完整 → null。 */
export function customBackendFromKeys(keys) {
  if (!keys || typeof keys !== 'object') return null
  const baseURL = String(keys[CUSTOM_KEY.baseURL] || '').trim()
  const model = String(keys[CUSTOM_KEY.model] || '').trim()
  const apiKey = String(keys[CUSTOM_KEY.apiKey] || '').trim()
  if (!baseURL || !model || !apiKey) return null
  const protocol = String(keys[CUSTOM_KEY.protocol] || '').trim().toLowerCase() === 'gemini' ? 'gemini' : 'openai'
  const needsProxy = String(keys[CUSTOM_KEY.needsProxy] || '').trim().toLowerCase() === 'true'
  return {
    name: CUSTOM_BACKEND_NAME,
    protocol,
    baseURL,
    model,
    keyNames: [CUSTOM_KEY.apiKey],
    needsProxy,
    stripThink: false,
  }
}

/** 自定义后端配置 hash（URL/模型/协议变化时缓存自动失效）。 */
export function customHashFor(cfg) {
  if (!cfg) return ''
  return sha256Text([cfg.baseURL, cfg.model, cfg.protocol, String(!!cfg.needsProxy)].join('|'))
}

// =============================================================
// 多路由自动发现（v1.2，B：换主模型后视觉仍可用）
// =============================================================
/** 纳入自动发现的模型家族前缀（DSH 官方文本模型家族）。
 *  deepseek-official 走替换式（无感）；其他家族路由自动注册 "(vision)" 变体。 */
export const SCOPE_FAMILIES = ['deepseek', 'glm']
/** 家族内原生视觉模型排除（不需要桥）。 */
export const VISION_MODEL_RE = /(deepseek-(vl|ocr)|janus|glm-[\d.]*v(\b|-))/i
/** 包装变体 providerId 前缀。 */
export const VISION_WRAP_PREFIX = 'vb-vision-'

/** 模型是否值得包装（家族内文本模型、非家族原生视觉模型）。 */
export function shouldWrapModelInfo(info) {
  if (!info || typeof info !== 'object') return false
  const id = String(info.id || info.name || '')
  if (!SCOPE_FAMILIES.some((f) => id.toLowerCase().startsWith(f))) return false
  if (VISION_MODEL_RE.test(id)) return false
  if (Array.isArray(info.inputModalities) && info.inputModalities.includes('image')) return false
  return true
}

/** provider 是否在本插件发现范围内（家族前缀，且非替换式主路由）。 */
export function providerInScope(providerId) {
  const id = String(providerId || '').toLowerCase()
  if (id === SCOPE_PROVIDER) return false
  return SCOPE_FAMILIES.some((f) => id.startsWith(f))
}
...
/** 缓存键：图片内容 + (问题⊕区域⊕请求后端⊕自定义后端hash⊕prompt/preprocess 版本)。
 *  region 不同、后端不同、自定义端点/模型变化、prompt 或预处理版本变化都必须区分；
 *  无问无区域无指定后端时 askHash 为空串（see_image 整图默认分析）。 */
export function cacheKeyFor(bytes, question, region, backend, customHash) {
  const ask = (question || '') + '\u0000' + (region || '') + '\u0000' + (backend || 'auto')
    + '\u0000' + (customHash || '') + '\u0000' + PROMPT_VERSION + '\u0000' + PREPROCESS_VERSION
 ...
 *  cfgOverride：自定义后端配置（见 customBackendFromKeys）；缺省用 BACKENDS 表。 */
export function buildBackendRequest(backendName, b64, mime, promptText, key, cfgOverride) {
  const cfg = cfgOverride || BACKENDS[backendName]
...
/** 按请求与可用凭证构建尝试链（v1.2：自定义后端优先，内置四家兜底）。
 *  每项含 { name, key, proxy, cfg }——cfg 供请求构建使用（内置表或自定义配置）。 */
export function backendTriesFor(requested, keys, proxy) {
  const tries = []
  const wanted = (name) => !requested || requested === 'auto' || requested === name
  // 自定义后端（VB_* 配置完整时在链头）
  const custom = customBackendFromKeys(keys)
  if (custom && wanted(CUSTOM_BACKEND_NAME)) {
    tries.push({ name: custom.name, key: custom.apiKey, proxy: custom.needsProxy ? proxy : undefined, cfg: custom })
  }
  // 内置四家兜底
  for (const name of BACKEND_ORDER) {
    ...
    if (!key) continue
    tries.push({ name, key, proxy: cfg.needsProxy ? proxy : undefined, cfg })
  }
  return tries
}
...
export const SEE_IMAGE_PARAMETERS = {
  ...
  properties: {
    ...
    backend: { type: 'string', enum: ['auto', 'custom', 'gemini', 'glm', 'groq', 'mimo'], description: '视觉后端：auto 依次尝试 自定义(VB_*) → MiMo → GLM → Groq → Gemini；custom 只用自定义端点；指定后端则强制使用' },
  },
  required: [],
}
...
export default {
  ...
  apply(ctx) {
    ...
    /** 预注描述缓存（独立键，带 prompt/自定义端点版本，不与 see_image 完整分析混用）。 */
    function describeCachePath(imageHash, customHash) {
      const suffix = customHash ? '-c' + customHash.slice(0, 8) : ''
      return path.join(rootDir, CACHE_PREFIX + imageHash + '-desc-v' + PROMPT_VERSION + suffix + '.json')
    }
    ...
    async function collectKeys() {
      ...
      // 自定义后端（VB_* 可选）
      for (const kn of Object.values(CUSTOM_KEY)) {
        if (keys[kn] === undefined) keys[kn] = await readKey(credentials, kn)
      }
    ...
    async function describeImageAsync(attachmentId, data, mediaType) {
      ...
      try {
        // 凭证与自定义后端配置（缓存键含 customHash：换 VB_* 后旧结果不命中）
        const { keys, proxy } = await collectKeys()
        const customCfg = customBackendFromKeys(keys)
        const cachePath = describeCachePath(imageHash, customHashFor(customCfg))
        try {
    ...
    async function resolveImageBlockAsync(ref, signal) {
      ...
      if (desc === undefined) {
        try {
          const { keys: k1 } = await collectKeys()
          const f = describeCachePath(imageHash, customHashFor(customBackendFromKeys(k1)))
    ...
    /** 本插件管理的视觉路由集合：替换式主路由 + 自动发现的包装变体路由。
     *  仅这些路由上的图片请求会被转换为描述文本；其他 provider（原生视觉等）原样放行。 */
    const visionRoutes = new Set([SCOPE_PROVIDER])

    /** 异步转换：仅作用于 visionRoutes；等待 in-flight 描述；并行解析图片块。
     *  maxChars 由调用方按模型上下文窗口计算（适配器从连接事实取）。 */
    async function convertImagesAsync(options, maxChars) {
      if (!options || !visionRoutes.has(options.provider)) return options
    ...
    ctx.effect(() => {
      ...
      // ---- v1.2 多路由自动发现（B）：其他文本路由自动注册 "(vision)" 变体 ----
      // deepseek-official 走替换式（上面）；本段处理家族内其他 provider
      // （如 glm 官方路由、本地 deepseek 兼容路由等）：注册包装 adapter
      // （新 providerId `vb-vision-<原id>`，模型选择器出现 "(vision)" 变体），
      // 变体声明 image 输入、stream 时把图片转描述后委托原路由。
      // 延迟注册的路由（如 settings 加载后才挂载）由 llm/adapters-updated 重扫覆盖。
      const wrapped = new Set()
      let sweeping = null
      async function sweepVisionWrappers() {
        if (typeof llm.listProviders !== 'function' || typeof llm.listModels !== 'function') return
        for (const info of llm.listProviders()) {
          const id = info && info.id
          if (!id || wrapped.has(id) || !providerInScope(id)) continue
          wrapped.add(id) // 先占位防并发重扫
          let models = []
          try {
            models = await llm.listModels(id)
          } catch {
            wrapped.delete(id) // 路由暂不可达，等下次拓扑变化重试
            continue
          }
          if (!models || !models.some(shouldWrapModelInfo)) {
            wrapped.delete(id)
            continue
          }
          const providerId = VISION_WRAP_PREFIX + id
          try {
            llm.registerAdapter([providerId], {
              providerInfo: (p) => ({ id: p, name: String(info.name || id) + ' (vision)' }),
              providerRetryPolicy: () => undefined,
              async listModels(_p, signal) {
                const ms = await llm.listModels(id, signal)
                return (ms || []).filter(shouldWrapModelInfo).map((m) => Object.assign({}, m, {
                  name: String(m.name || m.id) + ' (vision)',
                  inputModalities: (Array.isArray(m.inputModalities) ? m.inputModalities : []).concat(['image']),
                }))
              },
              async resolveModel(_p, model, signal) {
                const upstream = typeof llm.resolveModelInfo === 'function'
                  ? await llm.resolveModelInfo(id, model, signal)
                  : await llm.resolveModel(id, model, signal)
                if (!shouldWrapModelInfo(upstream)) {
                  throw new Error('model "' + model + '" is outside the vision wrap scope')
                }
                return Object.assign({}, upstream, {
                  id: model,
                  inputModalities: (Array.isArray(upstream.inputModalities) ? upstream.inputModalities : []).concat(['image']),
                })
              },
              stream(options) {
                return (async function* () {
                  const converted = await convertImagesAsync(options, options.contextWindow)
                  yield* llm.stream(Object.assign({}, options, { provider: id, messages: converted.messages }))
                })()
              },
            })
            visionRoutes.add(providerId)
            console.log('[vision-bridge] 已注册视觉包装路由 ' + providerId + '（模型选择器出现 "(vision)" 变体）')
          } catch (e) {
            console.error('[vision-bridge] 包装路由注册失败 ' + providerId + ': ' + (e && e.message ? e.message : String(e)))
            wrapped.delete(id)
          }
        }
      }
      const sweepOnce = () => {
        if (!sweeping) {
          sweeping = sweepVisionWrappers().catch((e) => {
            console.error('[vision-bridge] 路由发现扫描失败（将在下次拓扑变化重试）', e && e.message ? e.message : String(e))
      ...
      if (typeof ctx.on === 'function') {
        ctx.on('llm/adapters-updated', () => { void sweepOnce() })
      }
    ...
    async function callBackend(subprocessSvc, t, b64, mime, promptText, cwd, signal, proxy) {
      const { url, headers, bodyJson, stripThink } = buildBackendRequest(t.name, b64, mime, promptText, t.key, t.cfg)
    ...
    async function seeImage(args, exec) {
      ...
      const requested = args.backend === 'gemini' || args.backend === 'glm' || args.backend === 'groq' || args.backend === 'mimo' || args.backend === 'custom' ? args.backend : 'auto'
      ...
      // ---- 结果缓存（同图同问同区域同后端同版本不重复调用；自定义端点变化自动失效） ----
      const { keys, proxy } = await collectKeys()
      const customCfg = customBackendFromKeys(keys)
      const { imageHash, askHash } = cacheKeyFor(rawBytes, question, region, requested, customHashFor(customCfg))
      ...
      if (tries.length === 0) {
        const names = requested && requested !== 'auto' ? [requested] : BACKEND_ORDER.concat(CUSTOM_BACKEND_NAME)
        const wanted = names.map((n) => BACKENDS[n] ? BACKENDS[n].keyNames.join(' / ') : CUSTOM_KEY.apiKey).join(' 或 ')
      ...
      if (!chosen) {
        throw new Error('所有视觉后端均失败。\n' + errors.join('\n') +
          (requested === 'auto' ? '\n提示：auto 模式依次尝试 自定义(VB_*) → MiMo（直连）→ GLM（直连）→ Groq（走代理）→ Gemini（走代理）。Groq/Gemini 国内直连不通，需 VISION_PROXY 代理。限流会自动等待重试。' : ''))
      }
      ...
      if (!question && !region) {
        try {
          const descPath = describeCachePath(imageHash, customHashFor(customCfg))
    ...
    console.log('[vision-bridge] v1.2.0：自定义视觉后端（VB_*）+ 多路由自动发现（(vision) 变体）+ 包装 Adapter 架构；DSH 升级若导致注册失败，自检会明确告警。')
  },
}
